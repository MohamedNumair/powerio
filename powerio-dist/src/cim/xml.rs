//! The CIMXML (IEC 61970-552) subset CIM instance files use, on quick-xml's
//! namespace-aware pull parser. Distribution CIM (IEC 61968-13 / GridAPPS-D)
//! serializes with the same syntax as CGMES; this is a self-contained copy of
//! the transmission crate's reader so the two crates stay decoupled.
//!
                    ...
                    2 => {
                        if name == "md:FullModel" {
                            ...
                            for attr in start.attributes().with_checks(false).flatten() {
                                ...
                                if key.ends_with(b"ID") || key.ends_with(b"about") {
...
fn resource_attr(start: &quick_xml::events::BytesStart<'_>) -> Option<String> {
    for attr in start.attributes().with_checks(false).flatten() {
        if attr.key.as_ref().ends_with(b"resource") {
            return Some(normalize_id(&String::from_utf8_lossy(&attr.value)));
        }
    }
    None
}
